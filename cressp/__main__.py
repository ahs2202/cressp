...
def cressp( path_file_protein_target = None, path_file_protein_query = 'human', path_folder_output = 'default', n_threads = 1, l_window_size = [ 30 ], float_thres_e_value = 30, flag_use_HMM_search = False, path_file_query_hmmdb = 'human', flag_use_rcsb_pdb_only = False, int_number_of_proteins_in_a_batch_during_dnn_prediction = 1000, flag_only_use_structural_properties_of_query_proteins = False, float_thres_avg_score_blosum_weighted__b_cell = 0.15, float_thres_avg_score_blosum__b_cell = 0.0, float_thres_rsa_correlation = 0.0, float_thres_avg_blosum62_score_for_mhc = 2, float_thres_min_mhc_allele_frequency = 0.5, float_thres_binding_affinities_in_nM = 500, flag_replace_unconventional_acid_code = False, flag_use_gpu_devices = False, flag_use_all_gpu_devices = False, flag_deduplicate_based_on_aligned_subsequences_for_visualization = False ) :
    ...
    path_file_protein_target = None : (Required) an input FASTA file containing target protein sequences.
            
    path_file_protein_query = 'human' : (Default: UniProt human proteins) an input FASTA file containing query protein sequences. Alternatively
            
    path_folder_output = 'default' : (Default: a subdirectory 'cressp_out/' of the current directory) an output directory.
    ...
    path_file_query_hmmdb = 'human' : (Default: a HMM profile database of 1012 human proteins searched against UniProt Pan Proteomes. These proteins consist of experimentally validated human autoantigens) a file containing HMM DB of query proteins aligned against pan-proteomes.
    ...
    flag_only_use_structural_properties_of_query_proteins = False : (Default: False) Only use estimated structural properties of the query proteins when predicting cross-reactivity between target and query proteins (skip the estimation of structural properties of target proteins). When 'path_file_protein_query' == 'human' (default value), it will significantly reduce computation time by skipping estimation and prediction steps of structural properties of target and query proteins.
    ...
    if flag_usage_from_command_line_interface :
        ...
        parser.add_argument( "-t", "--path_file_protein_target", help = "(Required) an input FASTA file containing target protein sequences." )
        parser.add_argument( "-q", "--path_file_protein_query", help = "(Default: UniProt human proteins) an input FASTA file containing query protein sequences.", default = 'human' )
        parser.add_argument( "-o", "--path_folder_output", help = "(Default: a subdirectory 'cressp_out/' of the current directory) an output directory", default = "default" )
        ...
        parser.add_argument( "-d", "--path_file_query_hmmdb", help = "(Default: a HMM profile database of 1012 human proteins searched against UniProt Pan Proteomes. These proteins consist of experimentally validated human autoantigens) a file containing HMM DB of query proteins aligned against pan-proteomes", default = "human" )
        ...
        if args.path_file_protein_target is None :
...
#         path_file_protein_target_representative
        path_file_protein_target = args.path_file_protein_target
        path_file_protein_query = args.path_file_protein_query
        path_folder_output = args.path_folder_output
        path_file_query_hmmdb = args.path_file_query_hmmdb
    else :
        ''' parse arguments when the function was called from an interactive Python interpreter '''
        if path_file_protein_target is None :
    ...
    if path_file_protein_query == 'human' :
        ...
        path_file_protein_query = f'{path_folder_cressp}data/human/human_uniprot.fa' # set default 'path_file_protein_query' # file_name will be used to refer to the given query protein, and thus 'human_uniprot' is used as a file_name.
        if not os.path.exists( path_file_protein_query ) : # if default human protein fasta file is not available, extract the sequence from the dataframe
            df_protein_query = pd.read_csv( f'{path_folder_cressp}data/human/uniprot.tsv.gz', sep = '\t' )
        ...
        if flag_use_HMM_search : 
            if path_file_query_hmmdb == 'human' :
    ...
    elif  '.txt' in path_file_protein_query : 
        Info( "The query data is txt format. Change subset mode." )
        flag_default_protein_query_was_used = True
    ...
    """
    def __Get_File_Name_Without_Extension__( path_file ) :
        ''' get file_name of a given directory to a fasta file (gzipped or uncompressed) and return it '''
        name_file_without_extension = path_file.rsplit( '/', 1 )[ 1 ]
    ...
    # retrieve file names
    name_file_protein_query = __Get_File_Name_Without_Extension__( path_file_protein_query ) # retrieve file name containing the query proteins
    name_file_protein_target = __Get_File_Name_Without_Extension__( path_file_protein_target ) # retrieve file name containing the query proteins
    
    """ check flag """
    path_file_flag = f"{path_folder_pipeline}copying_input_files_completed.flag"
    if not os.path.exists( path_file_flag ) :
        
        try :
            dict_fasta_protein_query = FASTA_Read( path_file_protein_query )
            FASTA_Write( f"{path_folder_pipeline}protein_query.fasta", dict_fasta = dict_fasta_protein_query )
            path_file_protein_query = f"{path_folder_pipeline}protein_query.fasta" # set directory of fasta file to the new file location
        except :
            print( f"exiting due to an error while reading and moving 'path_file_protein_query' {path_file_protein_query}" )
            if flag_usage_from_command_line_interface : sys.exit( )
            else : return - 1

        try :
            dict_fasta_protein_target = FASTA_Read( path_file_protein_target )
            FASTA_Write( f"{path_folder_pipeline}protein_target.fasta", dict_fasta = dict_fasta_protein_target )
            path_file_protein_target = f"{path_folder_pipeline}protein_target.fasta" # set directory of fasta file to the new file location
        except :
            print( f"exiting due to an error while reading and moving 'path_file_protein_target' {path_file_protein_target}" )
            if flag_usage_from_command_line_interface : sys.exit( )
            else : return - 1
        ...
        path_file_protein_query = f"{path_folder_pipeline}protein_query.fasta" # set directory of fasta file to the new file location
        path_file_protein_target = f"{path_folder_pipeline}protein_target.fasta" # set directory of fasta file to the new file location
        dict_fasta_protein_query = FASTA_Read( path_file_protein_query ) # load protein sequences of query (required during BLASTP output processing)
    ...
    dict_cressp_setting = {  
        ...
        'path_file_protein_target' : path_file_protein_target,
        'path_file_protein_query' : path_file_protein_query,
        'path_folder_output' : path_folder_output,
        'path_file_query_hmmdb' : path_file_query_hmmdb,
            # internal setting
    ...
    path_file_json_setting_cressp = f"{path_folder_pipeline}cressp_setting.json"
    if os.path.exists( path_file_json_setting_cressp ) :
    ...
    with open( path_file_json_setting_cressp, 'w' ) as newfile :
    ...
    """
    """ check flag """
    path_file_flag = f"{path_folder_pipeline}blastp_completed.flag"
    if not os.path.exists( path_file_flag ) :

        # create blastp_db using query_protein sequences
        path_prefix_blastdb_protein_query = f"{path_folder_pipeline}makeblastdb_out/protein_query"
        os.makedirs( f"{path_folder_pipeline}makeblastdb_out/", exist_ok = True )  
        OS_Run( [ "makeblastdb", "-in", f"{path_folder_pipeline}protein_query.fasta", '-dbtype', 'prot', '-parse_seqids', '-max_file_sz', '1GB', '-out', path_prefix_blastdb_protein_query ], path_file_stdout = f"{path_prefix_blastdb_protein_query}.makeblastdb.stdout.txt", path_file_stderr = f"{path_prefix_blastdb_protein_query}.makeblastdb.stderr.txt", return_output = False ) # make blast db for protein_query
        ...
        path_file_blastp_output = f'{path_folder_pipeline}blastp.tsv.gz'
    ...
    """ check flag """
    path_file_flag = f"{path_file_matched}.write_completed.flag"
    if not os.path.exists( path_file_flag ) :
        # load blastp result
        dict_qacc_to_seq = FASTA_Read( path_file_protein_target ) # read query protein sequences
        ...
        df_blastp = BLAST_Read( path_file_blastp_output, dict_qaccver_to_seq = dict_qacc_to_seq ) 
        ...
        if flag_use_HMM_search : 
            ...
            df = PD_Select( HMMER_HMMSEARCH_Read_output( path_file_hmmsearch_output ), query_accession = set( dict_qacc_to_seq ) ) 
        ...
        df_matched.to_csv( path_file_matched, sep = '\t', index = False )
    ...
    """ check flag """
    path_file_flag = f"{path_folder_pipeline}rsa_estimation_completed.flag"
    if not os.path.exists( path_file_flag ) :
        ...
        if flag_default_protein_query_was_used :
            shutil.copyfile( f'{path_folder_cressp}data/human/uniprot.tsv.gz', f'{path_folder_pipeline}protein_query.tsv.gz' ) 
            # use previously calculated alignment between human proteins and RCSB_PDB structures
            PKG.Download_Data( "data/human/uniprot.blastp_rcsb_pdb.with_aligned_seq.filtered.tsv.gz", path_remote, name_package ) # download data (alignment between human proteins and RCSB_PDB structures)
        ...
        for name_file in [ 'protein_target' ] if flag_default_protein_query_was_used else [ 'protein_target', 'protein_query' ] : # skip prediction of query proteins if default query proteins are used
            ...
            if not os.path.exists( path_file_flag_2 ) :
                Estimate_structural_property( f'{path_folder_pipeline}{name_file}.fasta', n_threads, path_folder_pipeline, path_folder_pipeline_temp, flag_use_rcsb_pdb_only, int_number_of_proteins_in_a_batch_during_dnn_prediction, flag_use_gpu_devices, flag_use_all_gpu_devices )

                """ set flag_2 """
                with open( path_file_flag_2, 'w' ) as newfile :
                    newfile.write( 'completed\n' )
    ...
    if os.path.exists( path_folder_pipeline_temp ) :
        shutil.rmtree( path_folder_pipeline_temp )
    os.makedirs( path_folder_pipeline_temp, exist_ok = True )
    ...
    """ check flag """
    path_file_flag = f"{path_file_matched}.predicting_B_cell_cross_reactivity_completed.flag"
    if not os.path.exists( path_file_flag ) :
        
        Predict_B_cell_cross_reactivity( path_folder_pipeline, path_folder_pipeline_temp, n_threads, l_window_size, float_thres_e_value, flag_only_use_structural_properties_of_query_proteins, float_thres_avg_score_blosum_weighted__b_cell, float_thres_avg_score_blosum__b_cell, float_thres_rsa_correlation )
    ...
    """ check flag """
    path_file_flag = f"{path_file_matched}.predicting_T_cell_cross_reactivity_completed.flag"
    if not os.path.exists( path_file_flag ) :
        
        Predict_T_cell_cross_reactivity( path_folder_pipeline, float_thres_avg_blosum62_score_for_mhc, float_thres_min_mhc_allele_frequency, float_thres_binding_affinities_in_nM, flag_replace_unconventional_acid_code )
...
def Parse_Structural_Properties( path_file_df_sp, int_datatype = None, name_col_for_identifying_protein = 'id_protein' ) :
    """
    # 2021-05-31 20:29:02 
    ...
    'path_file_df_sp' : a file directory to TSV file containing structural properties estimated from CRESSP. Alternatively, a dataframe of the TSV file can be given as an input.
    ...
    """
    df_sp = pd.read_csv( path_file_df_sp, sep = '\t' ) if isinstance( path_file_df_sp, ( str ) ) else path_file_df_sp # read database of structural properties # if an object that is not a string datatype is given, assumes 'path_file_df_sp' is a dataframe containing structural properties
