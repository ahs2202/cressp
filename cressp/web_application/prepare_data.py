...
def Retrieve_Overlapping_Structures( path_file_input, path_file_b_cell, name_file_b_cell, path_folder_pipeline, path_folder_pipeline_temp ) :
    ...
    path_folder_pipeline_struc = f'{path_folder_pipeline}struc/' # a working directory of estimating structural properties
    
    # reetrieve start and end indices of lines
    int_index_line_start, int_index_line_end = pd.read_csv( path_file_input, sep = '\t', header = None ).values[ 0 ]
    ...
    newfile = gzip.open( f"{path_folder_pipeline_temp}{str_uuid}.{name_file_b_cell}.1.position_aligned_structure_added.tsv.gz", 'wb' ) # open an output file
    newfile.write( ( '\t'.join( l_col ) + '\n' ).encode( ) )
    
    ''' search maximum overlap with RCSB_PDB structures for each record in the input data '''
    index_line = -1
    with gzip.open( path_file_b_cell, 'rb' ) as file :
...
def Prepare_data_for_web_application( path_file_b_cell, path_file_t_cell, dict_cressp_setting ) :
    ...
    if flag_deduplicate_based_on_aligned_subsequences_for_visualization :
        ...
        path_file_b_cell_deduplicated = f"{path_file_b_cell.rsplit( '.tsv.gz', 1 )[ 0 ]}.deduplicated_by_subsequences.tsv.gz" # define an output file
        DF_Deduplicate_without_loading_in_memory( path_file_b_cell, path_file_b_cell_deduplicated, [ 'query_subsequence', 'target_subsequence' ], flag_header_is_present = True, str_delimiter = '\t' )
        path_file_b_cell = path_file_b_cell_deduplicated # use the deduplicated b-cell record file 
    
    """ Retrieve file names """
    name_file_b_cell = path_file_b_cell.rsplit( '/', 1 )[ 1 ].rsplit( '.tsv', 1 )[ 0 ] # retrieve name of the input file of b-cell
    name_file_t_cell = path_file_t_cell.rsplit( '/', 1 )[ 1 ].rsplit( '.tsv', 1 )[ 0 ] # retrieve name of the input file of t-cell
    ...
    df_blastp_pdb_query = pd.read_csv( f'{path_folder_pipeline_struc}protein_query.blastp_rcsb_pdb.with_aligned_seq.filtered.tsv.gz', sep = '\t' )
    ...
    Multiprocessing( l_bookmarks, Function = Retrieve_Overlapping_Structures, n_threads = len( l_bookmarks ), path_temp = path_folder_pipeline_temp, global_arguments = [ path_file_b_cell, name_file_b_cell, path_folder_pipeline, path_folder_pipeline_temp ] )
    # combine output files
    OS_FILE_Combine_Files_in_order( glob.glob( f"{path_folder_pipeline_temp}*.{name_file_b_cell}.1.position_aligned_structure_added.tsv.gz" ), f'{path_folder_pipeline_web}{name_file_b_cell}.1.position_aligned_structure_added.tsv.gz', overwrite_existing_file = True, flag_use_header_from_first_file = True, remove_n_lines = 1 )
    ...
    df_subsequence_pdb_web = pd.read_csv( f"{path_folder_pipeline_web}{name_file_b_cell}.1.position_aligned_structure_added.tsv.gz", sep = '\t' )
    PKG.Download_Data( "data/pdb/rcsb_pdb.label_seq_id.start_end.tsv.gz", path_remote, name_package ) # download data
    Map = MAP.Map( pd.read_csv( f"{path_folder_cressp}data/pdb/rcsb_pdb.label_seq_id.start_end.tsv.gz", sep = '\t' ).set_index( 'structure_id' ).int_index_residue_start.to_dict( ) ) # read structure_id -> 'label_seq_id' start position mapping
    ...
    df_subsequence_pdb_web.to_csv( f'{path_folder_pipeline_web}{name_file_b_cell}.2.residue_pos_corrected.tsv.gz', sep = '\t', index = False ) 
    ...
    df_subsequence_pdb_web = pd.read_csv( f'{path_folder_pipeline_web}{name_file_b_cell}.2.residue_pos_corrected.tsv.gz', sep = '\t', low_memory = False )
    df_mhc_web = pd.read_csv( path_file_t_cell, sep = '\t' )
    df_matched = pd.read_csv( f"{path_folder_pipeline}matched.tsv.gz", sep = '\t' )
    df_blastp_pdb_query = pd.read_csv( f"{path_folder_pipeline_struc}protein_query.blastp_rcsb_pdb.with_aligned_seq.filtered.tsv.gz", sep = '\t' )
    df_blastp_pdb_target = pd.read_csv( f"{path_folder_pipeline_struc}protein_target.blastp_rcsb_pdb.with_aligned_seq.filtered.tsv.gz", sep = '\t' )
    df_blastp_pdb_target[ 'id_alignment' ] = np.arange( len( df_blastp_pdb_target ) ) # retrieve integer index (id) of each alignment
    df_blastp_pdb_query[ 'id_alignment' ] = np.arange( len( df_blastp_pdb_query ) ) # retrieve integer index (id) of each alignment
    ...
    df_blastp_pdb_target_subset[ [ 'query_alignment', 'target_alignment' ] ].to_csv( f'{path_folder_pipeline_web}alignment_target_pdb.tsv', sep = '\t', index = False )
    df_blastp_pdb_query_subset[ [ 'query_alignment', 'target_alignment' ] ].to_csv( f'{path_folder_pipeline_web}alignment_query_pdb.tsv', sep = '\t', index = False )
    df_matched_subset[ [ 'query_alignment', 'target_alignment' ] ].to_csv( f'{path_folder_pipeline_web}alignment_query_target.tsv', sep = '\t', index = False )
    ...
    dict_fasta_query = FASTA_Read( f'{path_folder_pipeline}protein_query.fasta', header_split_at_space = True )
    dict_fasta_target = FASTA_Read( f'{path_folder_pipeline}protein_target.fasta', header_split_at_space = True )
    ...
    def __Read_Fasta_Header__( path_file_fasta ) :
    ...
    for df_acc, path_file_fasta in zip( [ df_acc_query, df_acc_target ], [  f'{path_folder_pipeline}protein_query.fasta',  f'{path_folder_pipeline}protein_target.fasta' ] ) :
        dict_fasta = FASTA_Read( path_file_fasta )
    ...
    # save intermediate results
    df_acc_query.to_csv( f'{path_folder_pipeline_web}acc_query.source.tsv.gz', sep = '\t', index = False )
    df_acc_target.to_csv( f'{path_folder_pipeline_web}acc_target.source.tsv.gz', sep = '\t', index = False )
    df_acc_pdb.to_csv( f'{path_folder_pipeline_web}acc_pdb.source.tsv.gz', sep = '\t', index = False )
    # save accessions for web application
    df_acc_query.to_csv( f'{path_folder_pipeline_web}acc_query.tsv', columns = [ 'value', 'n_counts', 'fasta_header', 'sequence' ], sep = '\t', index = False )
    df_acc_target.to_csv( f'{path_folder_pipeline_web}acc_target.tsv', columns = [ 'value', 'n_counts', 'fasta_header', 'sequence' ], sep = '\t', index = False )
    ...
    df_subsequence_pdb_web.to_csv( f'{path_folder_pipeline_web}BCellCrossReactivity.source.tsv.gz', sep = '\t', index = False ) # save an intermediate result
        ...
        for int_window_size in df.window_size.unique( ) : # write the BCellCrossReactivity data for each window_size
            ...
            for n_records in [ 1000, 2000, 5000, 10000, 20000, 40000, 80000, 160000 ] : # write the TCellCrossReactivity data for multiple numbers of records
                if len( df_for_a_window_size ) < n_records * float_thres_margin :
                    ...
                    df_for_a_window_size.to_csv( f'{path_folder_pipeline_web}BCellCrossReactivity.{name}.windowSize_{int_window_size}.top_{n_records}.tsv', sep = '\t', index = False, float_format = '%.3f' ) # write only 3 digits below floating point to reduce file size (except for floats in columns in 'l_col_float_scientific_notation')
                    break  # stop subsetting after writing all records as a file
                else : df_for_a_window_size.iloc[ : n_records ].to_csv( f'{path_folder_pipeline_web}BCellCrossReactivity.{name}.windowSize_{int_window_size}.top_{n_records}.tsv', sep = '\t', index = False, float_format = '%.3f' ) # write only 3 digits below floating point to reduce file size (except for floats in columns in 'l_col_float_scientific_notation')
    ...
    df_mhc_allele.to_csv( f'{path_folder_pipeline_web}mhc_allele.tsv', columns = [ 'value' ], sep = '\t', index = False )
    ...
    df_mhc_web.to_csv( f'{path_folder_pipeline_web}TCellCrossReactivity.source.tsv.gz', sep = '\t', index = False ) # save an intermediate result
    ...
    for df, name in zip( [ df_mhc_web ], [ 'all' ] ) :
        ...
        for n_records in [ 1000, 2000, 5000, 10000, 20000, 40000, 80000 ] : # write the TCellCrossReactivity data for multiple numbers of records
            if len( df ) < n_records * float_thres_margin :
                n_records = len( df )
                df.drop( columns = [ 'score_for_sorting' ] ).iloc[ : n_records ].to_csv( f'{path_folder_pipeline_web}TCellCrossReactivity.{name}.top_{n_records}.tsv', sep = '\t', index = False, float_format = '%.3f' ) # write only 3 digits below floating point to reduce file size (except for floats in columns in 'l_col_float_scientific_notation')
                break  # stop subsetting after writing all records as a file
            else : df.drop( columns = [ 'score_for_sorting' ] ).iloc[ : n_records ].to_csv( f'{path_folder_pipeline_web}TCellCrossReactivity.{name}.top_{n_records}.tsv', sep = '\t', index = False, float_format = '%.3f' ) # write only 3 digits below floating point to reduce file size (except for floats in columns in 'l_col_float_scientific_notation')
    ...
    # read entity records
    PKG.Download_Data( "data/pdb/rcsb_pdb.id_model_and_label_entity_id.tsv.gz", path_remote, name_package ) # download data
    df_pdb_entity = pd.read_csv( f"{path_folder_cressp}data/pdb/rcsb_pdb.id_model_and_label_entity_id.tsv.gz", sep = '\t', keep_default_na = False )
    ...
    df_acc_pdb.to_csv( f'{path_folder_pipeline_web}acc_pdb.source.tsv.gz', sep = '\t', index = False ) # update the data for the web_application
    df_acc_pdb.id_model = df_acc_pdb.id_model_from_resource.replace( -1, np.nan ) # replace -1 to np.nan, representing an invalid value
    df_acc_pdb.to_csv( f'{path_folder_pipeline_web}acc_pdb.tsv', columns = [ 'value', 'id_pdb', 'id_chain', 'id_model', 'id_entity' ], sep = '\t', index = False )
    ...
    df_blosum62.to_csv( f'{path_folder_pipeline_web}blosum62.tsv', sep = '\t', index = False )
    ...
    for df, filename, l_col in zip( [ df_matched_subset, df_blastp_pdb_query_subset, df_blastp_pdb_target_subset, df_fasta_acc_query_subset, df_fasta_acc_target_subset ], [ 'alignment_query_target', 'alignment_query_pdb', 'alignment_target_pdb', 'structural_property_query', 'structural_property_target' ], [ [ 'query_alignment', 'target_alignment' ], [ 'query_alignment', 'target_alignment' ], [ 'query_alignment', 'target_alignment' ], l_col_acc_query, l_col_acc_target ] ) : # dict_dict_it :
        ...
        df_compact.to_csv( f'{path_folder_pipeline_web}{filename}__compact__for_web_application.tsv'.format( filename = filename ), sep = '\t', index = False )
    ...
    path_folder_pipeline_web_tmp = f"{path_folder_pipeline_web}tmp/"
    path_folder_pipeline_web_base64 = f"{path_folder_pipeline_web}base64/"

    os.makedirs( path_folder_pipeline_web_tmp, exist_ok = True )
    os.makedirs( path_folder_pipeline_web_base64, exist_ok = True )
    for path_file in glob.glob( f"{path_folder_pipeline_web}*.tsv" ) : # export tsv files in the 'path_folder_pipeline_web' folder
        print( f"compressing {path_file} and export data for web application" )
        l = path_file.rsplit( '/', 1 )
        l.insert( 1, 'tmp' )
        path_file_binary = '/'.join( l ) + '.gz'
        with open( path_file_binary, 'wb' ) as newfile : # create gzipped file using command line
            result = subprocess.run( [ 'gzip', '-c', path_file ], stdin = PIPE, stdout = PIPE, stderr = PIPE )
            newfile.write( result.stdout )
        l[ 1 ] = 'base64'
        path_file_binary_base64 = '/'.join( l ) + '.gz.base64.txt'
    ...
    df = GLOB_Retrive_Strings_in_Wildcards( f"{path_folder_pipeline_web_base64}*.top_*.tsv.gz.base64.txt", retrieve_file_size = True )
    ...
    df_file = GLOB_Retrive_Strings_in_Wildcards( f"{path_folder_pipeline_web_base64}*.*.windowSize_*.top_*.tsv.gz.base64.txt", retrieve_file_size = True )
    ...
    df_file = GLOB_Retrive_Strings_in_Wildcards( f"{path_folder_pipeline_web_base64}*.*.top_*.tsv.gz.base64.txt", retrieve_file_size = True )
    ...
    # copy web viewer to 
    shutil.copyfile( f"{path_folder_cressp}web_application/cressp_web_viewer.html", f"{path_folder_output}CRESSP_Web_Viewer.html" )
    # write read-me file
    with open( f'{path_folder_output}readme.txt', 'w' ) as newfile :
        newfile.write( '\n'.join( [ 
