...
#         dict_blosum62[ aa_0, aa_1 ] = score
    ...
    dir_folder_cressp = pkg_resources.resource_filename( name_package, 'data/human/uniprot.tsv.gz' )
    ...
    if dir_file_query_protein == 'human' :
        ...
        dir_file_query_protein = f'{dir_folder_cressp}data/human/uniprot.fa' # set default 'dir_file_query_protein'
        if not os.path.exists( dir_file_query_protein ) : # if default human protein fasta file is not available, extract the sequence from the dataframe
            df_protein_query = pd.read_csv( f'{dir_folder_cressp}data/human/uniprot.tsv.gz', sep = '\t' )
            dict_fasta_protein_query = df_protein_query.set_index( 'uniprot_acc' ).seq.to_dict( )
            FASTA_Write( dir_file_query_protein, dict_fasta = dict_fasta_protein_query )
    ...
    dir_file_target_protein = os.path.abspath( args.dir_file_target_protein )
    dir_file_query_protein = os.path.abspath( args.dir_file_query_protein )
    dir_file_query_hmmdb = os.path.abspath( args.dir_file_query_hmmdb )
    ...
    dir_folder_output = os.path.abspath( args.dir_folder_output )
    ...
    """
    try :
        dict_fasta_protein_query = FASTA_Read( dir_file_query_protein )
        FASTA_Write( f"{dir_folder_pipeline}protein_query.fasta", dict_fasta = dict_fasta_protein_query )
        dict_fasta_protein_query = f"{dir_folder_pipeline}protein_query.fasta" # set directory of fasta file to the new file location
    except :
        print( f"exiting due to an error while reading and moving 'dir_file_query_protein' {dir_file_query_protein}" )
        sys.exit( )
        
    try :
        dict_fasta_protein_target = FASTA_Read( dir_file_target_protein )
        FASTA_Write( f"{dir_folder_pipeline}protein_target.fasta", dict_fasta = dict_fasta_protein_target )
        dict_fasta_protein_target = f"{dir_folder_pipeline}protein_target.fasta" # set directory of fasta file to the new file location
    except :
        print( f"exiting due to an error while reading and moving 'dict_fasta_protein_target' {dict_fasta_protein_target}" )
        sys.exit( )
    ...
    if flag_use_HMM_search : 
        ...
        if not flag_combined_search_result_exists : # if combine result does not exist
            OS_Run( [ 'hmmsearch', '-o', dir_file_hmmsearch_output, '--acc', '--notextw', '--cpu', f'{n_threads}', '-E', f'{float_search_thres_e_value}', dir_file_query_hmmdb, dict_fasta_protein_target ], dir_file_stdout = f"{dir_folder_pipeline}hmmsearch.stdout.txt", dir_file_stderr = f"{dir_folder_pipeline}hmmsearch.stderr.txt", return_output = False ) # run hmmsearch
    ...
    dir_file_combined = f'{dir_folder_pipeline}searched.tsv.gz'
    
    # load blastp result
    dict_qacc_to_seq = FASTA_Read( dict_fasta_protein_target ) # read query protein sequences
...
def Combine_Result_Files( dir_file_input ) :
    ...
    for arr_input in pd.read_csv( dir_file_input, sep = '\t', header = None ).values :
        ...
        dir_file_output_combining = f'{dir_folder_pipeline}subsequence__window_size_{int_window_size}__struc_data_human_only.combining.tsv.gz'
        dir_file_output_combining_completed = f'{dir_folder_pipeline}subsequence__window_size_{int_window_size}__struc_data_human_only.tsv.gz'
